...
if ($_POST) {
    ...
    if (empty($_POST["nome"]) || empty($_POST["email"])) {
        ...
        $pk_usuario = trim($_SESSION["pk_usuario"]);
        ...
        if ($foto["error"] != 4) {
            ...
            // $ext_permitidos = ["bmp","jpg","jpeg","png","jfif,"tif","tiff"];
            $extensao = pathinfo($foto["name"], PATHINFO_EXTENSION);
            // VERIFICA SE A EXTENSÃO DO ARQUIVO CONTÉM NO ARRAY EXT_PERMITITDOS
            if (in_array($extensao, $ext_permitidos)) {
                $novo_nome = hash("sha256", uniqid() . rand() .
                    $foto["tmp_name"]) . "." . $extensao;

                // MOVER O ARQUIVO PARA A PASTA "FOTOS" COMO O NOVO NOME
                ...
                $_SESSION["tipo"] = "error";
                $_SESSION["title"] = "Ops!";
                $_SESSION["msg"] = "Arquivo de imagem NÂO permitido.";
                header("Location: ./");
                exit;
            }
        } else {
